...
func _show_purchase() -> void:
	...
	_btn_buy.disabled = not can_afford
	_btn_buy.text     = "🔑  Buy Key & Open" if can_afford else "Not enough steps"
...
func _on_buy() -> void:
	if not SaveManager.spend_steps(KEY_COST):
		return
	var winner := ItemRegistry.roll_item()
	_show_spin(winner)

# ── Spin ──────────────────────────────────────────────────────────────────────

func _show_spin(winner: Dictionary) -> void:
...
func _build_track(winner: Dictionary) -> void:
	for c in _spin_track.get_children():
		c.queue_free()
	var track: Array = []
	for _i in WINNER_IDX:
		track.append(ItemRegistry.random_item())
	track.append(winner)
	for _i in 8:
		track.append(ItemRegistry.random_item())
	for item in track:
...
func _animate_spin(winner: Dictionary) -> void:
	...
	var end_x  := vw * 0.5 - (WINNER_IDX * CARD_SLOT + CARD_W * 0.5)
